...
# BASE_MAX_VELOCITY = 200 

# class TrajectoryExecutor(Node):
#     def __init__(self, port_handler, packet_handler):
#         super().__init__('trajectory_executor')
        
#         # 🌟 2. 외부에서 생성된 핸들러를 그대로 이어받음 (포트 중복 오픈 방지)
...
#         # 실행 준비
#         self.load_csv('waving_trajectory1.csv')
...
class TrajectoryExecutor(Node):
    ...
    def execute_trajectory(self):
        ...
        # 🌟 [수정됨] 재생 속도 배율 설정 (3.0 = 3배 느리게 재생)
        TIME_SCALE = 3.0 
        
        self.get_logger().info(f"🚀 손인사 시작! (현재 {TIME_SCALE}배 느리게 재생 중)")

        for t_target, angles in self.trajectory_data:
            # 🌟 [수정됨] 목표 시간에 TIME_SCALE을 곱해서 모터가 도달할 충분한 여유 시간을 줍니다.
            while (time.time() - start_time) < (t_target * TIME_SCALE):
                time.sleep(0.001)
